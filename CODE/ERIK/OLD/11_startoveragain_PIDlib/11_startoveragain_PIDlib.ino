...
short light_bar_values_binary_leftside_sum = 0 ;
short light_bar_values_binary_rightside_sum = 0 ;


float light_bar_values_raw_float_leftside_sum = 0 ;
float light_bar_values_raw_float_rightside_sum = 0 ;
float error_line_raw_float = 0 ;
...
void error_clalculator_line_simple() ;
...
void loop() 
{
  ...
  if( ( millis() - last_time_stamp ) >= sample_time )
  {
      ...
      //error_clalculator_line_simple() ;
      error_clalculator_line_raw_float() ;
      ...
      // GET INPUT
      //input = map( error_line_simple, -3 , 3 , -255 , 255 ) ;
      //input = error_line_simple ;  
      //input = error_line_raw_float ;
      input = map( error_line_raw_float, -250 , 350 , -90 , 90 ) ;
...
/*
      ...
      analogWrite( M1_IN_2 , -output ) ;
      analogWrite( M2_IN_1 , 0 ) ;
      analogWrite( M2_IN_2 , output ) ; 
*/
...
/*
      ...
      // ________ MOTOR FORMAN _______
      // IF ON CENTER
      if( error_line_simple == 0 )
      {
          // MOVE FORWARD
          analogWrite( M1_IN_1 , 0 ) ;
          analogWrite( M1_IN_2 , speed_level ) ;
          analogWrite( M2_IN_1 , 0 ) ;
          analogWrite( M2_IN_2 , speed_level ) ; 

          // HOLD UNTIL NOT
          //while( error_line_simple == 0 ) ;
...
// ______ ERROR CALCULATOR LINE SIMPLE () ______
...
void error_clalculator_line_simple()
{
  ...
  for ( a = 0 ; a < 13 ; a++ ) 
  {
      // IF LESS THAN THRESHOLD THAN ASSIGN A VALUE OF ONE ELSE ASSIGN AVALUE OF ZERO
      if( light_bar_values[ a ] <= 680 ) light_bar_values_binary[ a ]  = 1 ;   
      else light_bar_values_binary[ a ] = 0 ;
  }


  // ZERO-OUT SUMS
  light_bar_values_binary_leftside_sum = 0 ;
  light_bar_values_binary_rightside_sum = 0 ;
  

  // ADD UP LEFT SIDE
  for ( a = 0 ; a < 6 ; a++ ) light_bar_values_binary_leftside_sum = light_bar_values_binary_leftside_sum + light_bar_values_binary[ a ] ;   


  // ADD UP RIGHT SIDE
  for ( a = 7 ; a < 13 ; a++ ) light_bar_values_binary_rightside_sum = light_bar_values_binary_rightside_sum + light_bar_values_binary[ a ] ;


  // CALCULATE ERROR
  error_line_simple = light_bar_values_binary_leftside_sum - light_bar_values_binary_rightside_sum ;
...
// ______ ERROR CALCULATOR LINE RAW FLOAT () ______

void error_clalculator_line_raw_float()
{
...
*/
  ...
  // ZERO-OUT SUMS
  light_bar_values_raw_float_leftside_sum = 0 ;
  light_bar_values_raw_float_rightside_sum = 0 ;
  

  // ADD UP LEFT SIDE
  for ( a = 0 ; a < 6 ; a++ ) light_bar_values_raw_float_leftside_sum = light_bar_values_raw_float_leftside_sum + light_bar_values[ a ] ;   


  // ADD UP RIGHT SIDE
  for ( a = 7 ; a < 13 ; a++ ) light_bar_values_raw_float_rightside_sum = light_bar_values_raw_float_rightside_sum + light_bar_values[ a ] ;


  // CALCULATE ERROR
  error_line_raw_float = light_bar_values_raw_float_leftside_sum - light_bar_values_raw_float_rightside_sum ;
...
void print_statistics()
{
  ...
  for ( a = 0 ; a < 13 ; a++ ) 
  {
      Serial.print( light_bar_values[ a ] , 2 ) ; 
      Serial.print("\t") ;    
  }
...
*/
  ...
  for ( a = 0 ; a < 13 ; a++ ) 
  {
      Serial.print( light_bar_values_binary[ a ] ) ; 
      Serial.print("\t") ;    
  }


    // PRINT ENCODER PREVIOUS
    Serial.print( "E_simple: " ) ; 
    Serial.print( error_line_simple, 3 ) ; 
    Serial.print("\t") ; 


    // PRINT ENCODER PREVIOUS
    Serial.print( "E_raw_left: " ) ; 
    Serial.print( light_bar_values_raw_float_leftside_sum, 3 ) ; 
    Serial.print("\t") ; 



    // PRINT ENCODER PREVIOUS
    Serial.print( "E_raw_right: " ) ; 
    Serial.print( light_bar_values_raw_float_rightside_sum, 3 ) ; 
    Serial.print("\t") ; 



    // PRINT ENCODER PREVIOUS
    Serial.print( "E_raw_float: " ) ; 
    Serial.print( error_line_raw_float, 3 ) ; 
    Serial.print("\t") ; 
...
/*
    // PRINT COUPON
    Serial.print( "EncPC: " ) ; 
    Serial.print( enc_previous_coupon ) ; 
    Serial.print("\t") ; 


    // PRINT ENCODER PREVIOUS
    Serial.print( "EncP: " ) ; 
    Serial.print( enc_previous ) ; 
    Serial.print("\t") ; 


    // PRINT ENCODER DIFFERENCE
    Serial.print( "   EncDiff: " ) ; 
    Serial.print( enc1.read() - enc_previous ) ; 
    Serial.print("\t") ; 
...
/*
          // IF VALID FOR UPDATE
          if( enc_previous_coupon == 1 )
          {   
              // UPDATE PREVIOUS ENCODER VALUE
              enc_previous = enc1.read();
    
              // TAKE COUPON
              enc_previous_coupon = 0 ;
          }

          
          // MOVE RIGHT TWO CLICKS
          if( ( enc_previous - enc1.read() ) > 2 )
          {
              // MOTOR COMMANDS
              analogWrite( M1_IN_1 , 0 ) ;
              analogWrite( M1_IN_2 , 0 ) ;
