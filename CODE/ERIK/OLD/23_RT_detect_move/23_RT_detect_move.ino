...
- Inspect why wheel two delays in rotating in D-move.
...
// OVERALL
  ...
  float twinky_one_speed_const = 0.10 ;
...
// SPECIAL DETECT
  ...
  bool go_for_rt = 1 ;  // <-- WILL BECOME A FUNCTIN CALL TO MICHAELS MODULE
  bool go_for_st = 0 ;  // <-- WILL BECOME A FUNCTIN CALL TO MICHAELS MODULE
...
*/
    ...
    if( rt_final_detect == 1 && go_for_rt == 1 )         // <-- function calls to desicsion module in place of variables.
    {
...
void pid_vl_control()
{
    ...
    { 
        ...
        if( rt_move_latch == 1 )
        {
            ...
            // STOP RIGHT WHEEL  <-- WISH THIS WAS A GRADUAL DECREASE
            twinky_two = rt_enc_two_final_detect ;

            //Serial.print( "   IN_ML   " ) ;

            
            if ( rt_wheel_one_mv_complt == 0 )
            {
                ...
                // TURN WHEEL ONE FORWARD A SPECIFIC AMOUNT
                //if( twinky_one - ( rt_enc_one_final_detect + 200 ) < 0 )
                if( enc1.read() - rt_enc_one_final_detect < 200 )
                {  
...
void printout()
{
  ...
  for ( a = 0 ; a < 13 ; a++ ) 
  {
    ...
    // PRINT
    Serial.print( "    t1 - ( tenc_fin + 200): " ) ; 
    Serial.print( twinky_one - ( rt_enc_one_final_detect + 200 ) ) ; 
