...
void special_detect()
{
    ...
    if( d_detected == 1 ) 
    {
        ...
        if( d_enc_cupon == 0 ) 
        {
          ...
          d_enc_two_prev = enc2.read() ;
          
        }


        // IF HAS BEEN A DEAD END FOR SOME ROTATION
        if( ( ( enc1.read() - d_enc_one_prev > 40 ) || ( enc2.read() - d_enc_two_prev > 40 ) ) && ( d_mv_latch == 0 ) )
        {
    ...
    {
        // YES RT SIGNATURE
        rt_initial_detected = 1 ;   
    }

    // NO RT SIGNITURE
    else rt_initial_detected = 0 ;
        



    // IF DETECTED
    if( rt_initial_detected == 1 ) 
    {
        ...
        if( rt_enc_cupon == 0 ) 
        {
          ...
          rt_enc_two_initial = enc2.read() ;
          
        }


        // IF HAS BEEN A RIGHT TURN FOR SOME ROTATION
        if( ( ( enc1.read() - rt_enc_one_initial > 10 ) || ( enc2.read() - rt_enc_two_initial > 10 ) ) && ( rt_initial_latch == 0 ) )
        {

            // GRAB INITIAL LATCH
            rt_initial_latch = 1 ;   // <-- HERE WE WILL NEED TO PUT IN MANUAL FORWARD UNTILL FINAL DETECTION        
    ...
    if( final_detect == 1 && go_for_rt == 1 )
    {

        // LATCH RIGHT MOVE
        rt_move_latch = 1 ;

        // PERFORM RIGHT TURN MANUVER
        rt_move() ;
    ...
    else if( final_detect == 1 && go_for_st == 1 )
    {
        // RESET RT DETECT CONDITIONS
        rt_enc_cupon = 0 ;
        rt_initial_detected = 0 ;
        rt_initial_latch = 0 ;
        rt_final_detect = 0 ;
...
void command_motors()
{
    ...
    {
        analogWrite( M1_IN_1 , 0 /*whl_1_vl_PID_out * -1*/ ) ; // <-- ONE MUST HARD-CODE FLIP THE MAGNITUDE OF THE PWM INTO POSITIVE BECAUSE THE PWM RANGE IS 0 --> 255 (ALL POSITIVE).
...
void printout()
{
  ...
  for ( a = 0 ; a < 13 ; a++ ) 
  {
    ...
    // PRINT
    Serial.print( "    rt_detected: " ) ; 
    Serial.print( rt_initial_detected ) ; 
    ...
    // PRINT
    Serial.print( "    rt_fn_lch: " ) ; 
    Serial.print( rt_final_latch ) ; 
