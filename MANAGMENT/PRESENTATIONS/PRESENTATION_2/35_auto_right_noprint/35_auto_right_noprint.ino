...
*/
  ...
  float line_lf_PID_KP = twinky_speed_const / (float)500 ;
...
void decision_recv()
{
    ...
    // IF TEE OR RIGHT OR OPEN_LEFT
    if( ( flag_t == 1 ) || ( flag_r == 1 ) || ( flag_ol ) ) decision = 2 ;

    // IF LEFT
    else if( ( flag_l == 1 ) && ( flag_ol == 0 ) ) decision = 2 ;
    
    // IF DEAD END
    else if( flag_de == 1 ) decision = 4 ;

    // IF FINISH
    else if( flag_f == 1 ) decision = 5 ;
...
void pid_lf_control()
{
        ...
        else if( out_detect == 1 )
        {
            ...
            twinky_one_speed = twinky_speed_const / 2 ;
            twinky_two_speed = twinky_speed_const / 2 ;
...
// **************** THE PID Kp Ki Kd NEED TO BE PROPORTIONAL TO THE MAX TWINKY SPEEED !!!!!!!  *******************
